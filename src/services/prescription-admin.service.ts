...
export async function processPrescriptionUpload(
  userId: string,
  buffer: Buffer,
  mimeType: string,
  imageUrl: string,
  ...
  try {
    ...
    return { prescription, medicines: scheduledMedicines, dailyRoutine: routine };
  } catch (error) {
...
export async function applyPrescriptionToUser(prescriptionId: string) {
  const prescription = await Prescription.findById(prescriptionId);
  if (!prescription) {
    throw new Error("Prescription not found");
  }

  if (!prescription.extractedMedicines?.length) {
    throw new Error("No medicines to apply");
  }

  const user = await User.findById(prescription.userId).select("dailyRoutine");
  const routine = mergeDailyRoutine(user?.dailyRoutine ?? undefined);
  const scheduledMedicines = prescription.extractedMedicines.map((medicine: ExtractedMedicine) =>
    enrichMedicineWithSchedule(medicine, routine)
  );

  const result = await bulkCreateMedicinesAndReminders(
    prescription.userId.toString(),
    scheduledMedicines,
    { prescriptionId: prescription._id.toString(), dailyRoutine: routine }
  );
